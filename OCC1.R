...
#----Occupant-Centric Controls Function 1: Building Level First/Last Arrival and Last Departure----
...
# Create empty lists for loop (10 different samples for 9 occupants)
tim_metrics <- list()
arrive_cdf_forPlot <- list()
depart_cdf_forPlot <- list()

# Loop for 10 different samples of 9 occupants
for(k in 1:10){
  ...
  dat <- cbind.data.frame(dateDat,rowSums(allRooms[,sample_ind]))
  
  colnames(dat) <- c("Time","md")
  ...
  for (i in c(year(dat[1,1]):year(dat[nrow(dat),1]))){
    ...
    for (j in c(yday(dat[which(year(dat[,1])==i)[1],1]):yday(dat[max(which(year(dat[,1])==i)),1]))){
      ...
      temp <- subset(dat[,2], (yday(dat[,1]) == j) & (year(dat[,1]) == i))
  ...
  wdayIndex <- wday(levels(cut(dat$Time, breaks = "1 day")))
  
  # Isolate weekday arrivals
  arrive <- arrive[which(wdayIndex %in% c(2:6))]
  
  # Isolate weekday departures
  depart <- depart[which(wdayIndex %in% c(2:6))]
  
  # Convert the arrival times into number between 0 and 24
  arrive <- arrive*timestep_min/60
  
  # Convert the departure times into number between 0 and 24
  depart <- depart*timestep_min/60
    
  # Calculate the density of the departure times to plot
  departure_density <- density(depart, na.rm = T, adjust = 1.5)
  
  # Calculate the density of the arrival times to plot
  arrival_density <- density(arrive, na.rm = T, adjust = 1.5)
  
  # Create custom colours for histogram with reduced opacity to see overlap
  myred <- rgb(255, 0, 0, max = 255, alpha = 255*(0.2), names = "myred")
  myblue <- rgb(0, 0, 255, max = 255, alpha = 255*(0.2), names = "myblue")
  ...
  tim_metrics[[length(tim_metrics)+1]] <- c(colnames(allRooms)[sample_ind],arrive_erly,arrive_lat,depart_lat)
  
}

# Output a .csv containing the arrival cumulative distribution function for plotting
arrive_cdf_forPlot <- as.data.frame(matrix(unlist(arrive_cdf_forPlot), ncol = k, byrow = F))
colnames(arrive_cdf_forPlot) <- c(1:k)
write.csv(arrive_cdf_forPlot,"bldg_arrive_cdf.csv", row.names = F)

# Output a .csv containing the departure cumulative distribution function for plotting
depart_cdf_forPlot <- as.data.frame(matrix(unlist(depart_cdf_forPlot), ncol = k, byrow = F))
colnames(depart_cdf_forPlot) <- c(1:k)
write.csv(depart_cdf_forPlot,"bldg_depart_cdf.csv", row.names = F)
...
colnames(tim_metrics) <- c(1:k)
rownames(tim_metrics) <- c(c(1:9), "Earliest Arrival","Latest Arrival","Latest Departure")
write.csv(tim_metrics,"bldg_arrive_depart_lim.csv")